...
def Perspective_warping(im1, im2, im3):
        ...
        # creating space around image 1
        im1 = cv2.copyMakeBorder(im1,200,200,500,500, cv2.BORDER_CONSTANT)
        #transform image 2 into image 1's plane
        (M, pts1, pts2, mask) = getTransform(im2, im1, "homography")
        out1 = cv2.warpPerspective(im2, M, (im1.shape[1],im1.shape[0]), dst=im1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
        (M, pts1, pts2, mask) = getTransform(im3, out1, "homography")
        out2 = cv2.warpPerspective(im3, M, (out1.shape[1],out1.shape[0]), dst=out1.copy(),borderMode=cv2.BORDER_TRANSPARENT)
        plt.imshow(out2, cmap='gray')
        plt.show()

        #print RMSD(1, out2, cv2.imread("example_output1.png",0))
...
def Cylindrical_warping(img1, img2, img3):
        ...
        (M, pts1, pts2, mask) = getTransform(imgcyl2, imgcyl1,"affine")
        img2_warp = cv2.warpAffine(imgcyl2, M, (imgcyl1.shape[1], imgcyl1.shape[0]))
        img2_warp_mask = cv2.warpAffine(mask2, M, (imgcyl1.shape[1], imgcyl1.shape[0]))
        img2_warp_mask
        out = imgcyl1
        out[img2_warp_mask != 0] = img2_warp[img2_warp_mask!=0]
        (M, pts1, pts2, mask) = getTransform(imgcyl3, out, 'affine')

        img3_warp = cv2.warpAffine(imgcyl3, M, (imgcyl1.shape[1], imgcyl1.shape[0]))
        img3_warp_mask = cv2.warpAffine(mask3, M, (imgcyl1.shape[1], imgcyl1.shape[0]))

        #out = img3_warp
        out[img3_warp_mask != 0] = img3_warp[img3_warp_mask!=0]
